...
<link rel="stylesheet" type="text/css"
	href="${contextPath}/css/entrust.css" />
<script src="${contextPath}/js/entrust.js"></script>
...
$(document).on("click","i[name=add_row_btn]",function(){
		...
		var addStaffText = '<tr name="stuff">'
			...
			+'<td><input type="number" name="product_cnt" min=0 max=100></td>'
		+'</tr>';
...
<body>
	...
	<div class="container">
		<div class="exit_button">
			<button type="button" class="exit_btn" id="exit_btn" name="exit_btn"
				onClick="location.href='/main'">나가기</button>
		</div>
		...
		<div class="main">
			<div class="main_board">
				<form id="regForm" class="regForm" action="${pageContext.request.contextPath}" method="POST">
					...
					<div class="content2" id="content2" style="display: none;">
						...
						<div class="product_size_1">
							<input type="number" name="product_size" min=0 max=100>개
						</div>

						<div class="content2_header">박스에 들어가지 않는 물건이라면 이곳에 입력해주세요.</div>
						<div class="product_size_2">
							자전거 이상 크기 <input type="number" name="product_size" min=0
								max=100>개
						</div>
						<div class="product_size_3">
							싱글 침대 이상 크기 <input type="number" name="product_size" min=0
								max=100>개
						</div>
					</div>
					...
					<div class="content4" id="content4" style="display: none;">
						...
						<div class="deals">
							<div class="deal_1">
								<input type="radio" class="radio_btn, c_btn" id="direct_deal" name="trade_type_answer" value="직거래" />
								<label for="direct_deal"> 직거래 </label>
							</div>
							<div class="deal_2">
								<input type="radio" class="radio_btn, c_btn" id="delivery" name="trade_type_answer" value="택배" />
								<label for="delivery"> 택배 </label>
							</div>
						</div>
						<h2 class="head_5">추가 전달 내용을 입력해주세요</h2>
						<textarea class="post_contents" name="post_contents" rows="8" cols="60" placeholder="맡길 물건에 대한 간단한 설명을 입력해주세요" maxlength="1000"></textarea>
					</div>
					<div class="content5" id="content5" style="display: none;">
						...
						<div class="photo_type2">
							...
							<label for="photo_etc2"> <img id="etc_2" class="photo_etc" src="${contextPath}/image/photo_etc.jpg" />
							</label> <input type="file" id="photo_etc2" class="input_p" name = "photo" accept=".jpg, .png" onchange="readURL4(this);" />
							<label for="photo_etc3"> <img id="etc_3" class="photo_etc" src="${contextPath}/image/photo_etc.jpg" />
							</label> <input type="file" id="photo_etc3" class="input_p" name = "photo" accept=".jpg, .png" onchange="readURL5(this);" />
						</div>
				...
				<div class="content6" id="content6" style="display: none;">
					...
					<textarea class="caution" name="caution" rows="6" cols="60" disabled readonly="0">호스트와 직접 거래를 하시지 마시고 반드시 사이트를 통해 결제를 하시길 바랍니다. 또한 물건 보관 연장이 필요할 경우, 반드시 사이트를 통해 연장을 하시길 바랍니다. 물품 보관 만료 일주일전에 문자를 발송해 드립니다. </textarea>
					<div class="confirm">
						<ul class="content6_ul">
							<li class="content6_li">위 내용을 확인했습니다.
							<input id="submit_btn" type="checkbox" class="check_btn" /></li>
						</ul>
					</div>
					<button class="register_btn" onClick="submitForm()">물품 보관
						요청</button>
				</div>

				<div class="content7" id="content7" style="display: none;">
					<h2 class="head_7">완료됨</h2>
					<h4 class="head_7_1">
						호스트에게 물건 맡기기 신청을 완료하였습니다.<br> 호스트가 확인하는대로 알림을 드리겠습니다.
					</h4>
					<button class="finished"
						onClick="loacation.href='/main'">확인</button>
				</div>
			</div>
			<div id="bar">
				<div id="percent" style="height: 35px; width: 0%">0%</div>
			</div>
		</div>
	</div>

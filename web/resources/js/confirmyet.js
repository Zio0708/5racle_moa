...
//				str +=    '</div>'
				...
				str +=    '<button type="button" name="confirm_btn" id="confirm_btn" class='+data.requestId+'>승인</button>'
				str +=  '</div>'
				str +=  '<div class="rs_refuse_button">'
				str +=    '<button type="button" name="refuse_btn" id="refuse_btn" class='+data.requestId+'>거절</button>'
				str +=  '</div>'
				str +='</div>'
				str +='</div>'
				str +='</div>'
...
$(document).on('click','#approve_btn, #confirm_btn',function(){
...
$(document).on('click','#reject_btn,#refuse_btn',function(){
	...
	$.ajax({
		...
		success:function(result){
			...
			for(let i = result.pagination.curListCnt-1; i >= 0; i--){
				...
				$('<img/>',{src : "/resources/image/navbar/"+result.list[i].profileImg ,alt:"이미지 경로 오류"}).appendTo(td1);
				...
				$('<span/>',{id:"approve_btn", class:"approve_btn",text:"승인 ", class:result.list[i].articleNum}).appendTo(td6);
				$('<span/>',{text:" | "}).appendTo(td6);
				$('<span/>',{id:"reject_btn",class:"reject_btn",text:" 거절",class:result.list[i].articleNum}).appendTo(td6);
			}
